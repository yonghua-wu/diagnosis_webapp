...
const modelName = "electricalPatrol";
 ...
 * 巡逻计划信息
 */
export interface MechanicalElectricalPlanModel {
  // 创建操作人
  createdBy: string;
  // 创建操作人ID
  createdId: string;
...
export interface CreateMechanicalElectricalPlanModel {
  // 周期性计划路线
 ...
 * 分页查询参数
 */
export interface MechanicalElectricalPlanQueryModel {
  // 工作组ID
  groupId?: string;
  // 工作组名称
  groupName?: string;
...
export default class MechanicalElectricalPlan {
  public static async page(current: number, pageSize: number, query?: MechanicalElectricalPlanQueryModel) {
    const res = await http.post<HttpResponseBase<PaginationModel<MechanicalElectricalPlanModel[]>>>(`/${modelName}/getPatrolPlanPage`, {
      ...query,
      pageNo: current,
      pageSize: pageSize,
    });
  ...
  public static async detail(id: string) {
    const res = await http.post<HttpResponseBase<MechanicalElectricalPlanModel>>(`/${modelName}/getPatrolPlanDetail`, {
      id,
    });
    if (res.code === 200) {
      return res.data;
  ...
  public static async create(data: CreateMechanicalElectricalPlanModel) {
  ...
  public static async update(data: Partial<CreateMechanicalElectricalPlanModel>) {
