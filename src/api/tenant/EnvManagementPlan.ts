 ...
 * 巡逻计划信息
 */
export interface EnvironmentPlanModel {
  // 创建操作人
  createdBy: string;
  // 创建操作人ID
  createdId: string;
...
export interface CreateEnvironmentPlanModel {
  // 周期性计划路线
 ...
 * 分页查询参数
 */
export interface EnvironmentPlanQueryModel {
  // 工作组ID
  groupId?: string;
  // 工作组名称
  groupName?: string;
...
export default class EnvironmentPlan {
  public static async page(current: number, pageSize: number, query?: EnvironmentPlanQueryModel) {
    const res = await http.post<HttpResponseBase<PaginationModel<EnvironmentPlanModel[]>>>(`/${modelName}/getPatrolPlanPage`, {
      ...query,
      pageNo: current,
      pageSize: pageSize,
    });
  ...
  public static async detail(id: string) {
    const res = await http.post<HttpResponseBase<EnvironmentPlanModel>>(`/${modelName}/getPatrolPlanDetail`, {
      id,
    });
    if (res.code === 200) {
      return res.data;
  ...
  public static async create(data: CreateEnvironmentPlanModel) {
  ...
  public static async update(data: Partial<CreateEnvironmentPlanModel>) {
