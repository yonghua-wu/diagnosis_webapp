<template>
  <WorkGuestOriented v-model="activeTab" :tabs="tabs">
    ...
    <WorkSomethingDetail ref="refTicketDetail" v-show="activeTab === 0" :id="ticketId" :type="'REPORT'"></WorkSomethingDetail>
...
export default defineComponent({
  ...
  setup(props) {
    const tabs = ["报事详情", "评价详情", "回访详情"];
    const activeTab = ref(props.defaultTab);
    const route = useRoute();
    const ticketId = route.params.id as string;
    const router = useRouter();
    const addWorkOrder = () => {
      router.push({ name: "ResidentCreateReportAssociatedWorkOrder", params: { ticketId } });
    };
