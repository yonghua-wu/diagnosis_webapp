<template>
  ...
  >
    <template #breadcrumb>
      <Breadcrumb :items="['客户服务', '客户报事', '报事回访']" />
    </template>
  </SfTablePage>
</template>
<script lang="ts">
...
import ReportRevisit, { ReportRevisitModel } from "@/api/tenant/ReportRevisit";
...
export default defineComponent({
  ...
  setup() {
    ...
    const { formatLoadData } = useFormatData<ReportRevisitModel>((item) => {
      return {
        key: item.visitId,
        data: item,
        ...
        visitTime: moment(item.visitTime).format("YYYY-MM-DD"),
      };
    });
    ...
    const loadData = (current: number, pageSize: number) => {
      return ReportRevisit.page(current, pageSize, formatQueryData(fQueryData.value));
    };
    const refSfTablePage = ref();
    const router = useRouter();
    const toDetail = (record: any) => {
      router.push({ name: "ResidentReportRevisitDetail", params: { id: record.ticketId, model: "View" } });
    };
    return {
      refSfTablePage,
      tableColumns,
