<template>
  <div class="inspection-plan-detail container">
    <Breadcrumb :items="['环境管理', { name: '维保计划', router: () => $router.replace({ name: 'EnviromentMaintenancePlan' }) }, pageModelName]" />
          ...
          <a-space direction="vertical" fill>
            <a-space direction="vertical" fill>
              ...
              <a-row class="section-body">
                ...
                <a-col :span="8">
                  <a-form-item :rules="rules.jobId" field="jobId" label="维保事件" :row-props="{ gutter: 24 }">
                    <a-select
...
import MaintenancePlan, { CreateMaintenancePlanModel, CyclicalPlanTaskModel } from "@/api/tenant/EnvMaintenancePlan";
  ...
  setup() {
    ...
    const loadData = () => {
      ...
      MaintenancePlan.detail(route.params.id as string)
        .then((res) => {
          ...
          formData.value.addRouteSpots = res.cyclicalMaintenancePlaces;
    ...
    const createData = () => {
      ...
      const submitData: CreateMaintenancePlanModel = {
        addCyclicalPlanPlaces: formData.value.addRouteSpots,
      ...
      return MaintenancePlan.create(submitData)
        .then(() => {
          router.push({ name: "EnviromentMaintenancePlan" });
        })
        .catch((err) => {
    ...
    const updateData = () => {
      ...
      const submitData: CreateMaintenancePlanModel = {
        addCyclicalPlanPlaces: formData.value.addRouteSpots.map((item: any) => {
          item.planPlaceId && delete item.planPlaceId;
          return item;
        }),
        addCyclicalPlanTasks: planTasks,
        deleteCyclicalPlanPlaces: formData.value.deleteCyclicalPlanPlaces,
    ...
    Staff.findByServiceClass(formData.value.serviceClass).then((res) => {
      staffMap.value = {};
      staffList.value = res
      ...
      if (staffList.value.length === 0) {
        Modal.warning({
          title: "未配置环境主管",
          content: "需要先在员工管理添加环境主管才能继续进行操作。",
          closable: true,
        });
      }
    });
