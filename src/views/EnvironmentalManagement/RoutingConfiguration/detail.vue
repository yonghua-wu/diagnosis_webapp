<template>
  <div class="envmanagement-route-detail container">
    <Breadcrumb
      :items="['环境管理', { name: '工作路线配置', router: () => $router.replace({ name: 'EnviromentRoutingConfiguration' }) }, pageModelName]"
    />
    <a-spin :loading="loading" style="width: 100%">
      <CUVue :showModalTips="showModalTips">
        ...
        >
          ...
          <a-form-item :rules="rules.addRouteSpots" :validate-trigger="['input']" field="addRouteSpots" label="工作地点">
...
import EnvManagementRoute, { CreateRouteModel, UpdateRouteModel } from "@/api/tenant/EnvManagementRoute";
...
export default defineComponent({
  ...
  setup() {
    ...
    const loadData = () => {
      loading.value = true;
      stopWatch.value?.();
      EnvManagementRoute.detail(route.params.id as string)
        .then((res) => {
          formData.value = res;
    ...
    const createData = () => {
      const submitData: CreateRouteModel = {
      ...
      return EnvManagementRoute.create(submitData)
        .then(() => {
          router.push({ name: "EnviromentRoutingConfiguration" });
        })
        .finally(() => {
    ...
    const updateData = () => {
      const submitData: UpdateRouteModel = {
      ...
      console.log("submitData", submitData);
      return EnvManagementRoute.update(submitData)
        .then(() => {
          router.push({ name: "EnviromentRoutingConfiguration" });
        })
        .finally(() => {
