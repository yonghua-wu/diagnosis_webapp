<template>
  <div class="envmanagement-plan-detail container">
    <Breadcrumb :items="['环境管理', { name: '日常工作计划', router: () => $router.replace({ name: 'EnviromentPatrolPlan' }) }, pageModelName]" />
      ...
      <CUVue :showModalTips="showModalTips">
        ...
        >
          <a-space direction="vertical" fill>
              ...
              <a-row class="section-body" :gutter="24">
                <a-col :span="8">
                  <a-form-item :rules="rules.planName" field="planName" label="名称" :row-props="{ gutter: 24 }">
                    <a-input :max-length="50" v-model="formData.planName" placeholder="请输入工作计划名称" />
                  </a-form-item>
                </a-col>
                <a-col :span="8">
                  <a-form-item required label="事件" :row-props="{ gutter: 24 }">
                    <a-select
                      placeholder="请选择事件"
            ...
            <a-space direction="vertical" fill>
              <a-row class="section-title">工作路线选择</a-row>
              <a-row class="section-body" :gutter="24">
                <a-col :span="8">
...
import QualityCheckPlan, { CreateEnvironmentPlanModel, CyclicalPlanTaskModel } from "@/api/tenant/EnvManagementPlan";
import { deepClone } from "@/utils/utils";
import EnvManagementRoute, { RouteModel } from "@/api/tenant/EnvManagementRoute";
...
import { Modal, SelectOption } from "@arco-design/web-vue";

export default defineComponent({
  name: "envmanagement-plan-detail",
  components: {
    Breadcrumb,
    CUVue,
  },
  setup() {
    ...
    const rules = {
      ...
      cyclicalPlanRoutes: new RulesLib().required("请选择工作路线").done(),
    };
    ...
    JobConfig.all({ serviceClass: 2, serviceType: 1, moduleId: 5 })
      .then((res) => {
        ...
        return JobConfig.all({ serviceClass: 2, serviceType: 1, moduleId: 6 }).then((res) => {
    ...
    const createData = () => {
      ...
      const submitData: CreateEnvironmentPlanModel = {
      ...
      return QualityCheckPlan.create(submitData)
        .then(() => {
          router.push({ name: "EnviromentPatrolPlan" });
        })
        .catch((err) => {
    ...
    if (formData.value.serviceClass) {
      EnvManagementRoute.getAll({ serviceClass: formData.value.serviceClass }).then((res) => {
      ...
      Staff.findByServiceClass(formData.value.serviceClass)
        .then((res) => {
          ...
          if (staffList.value.length === 0) {
            Modal.warning({
              title: "未配置环境人员",
              content: "需要先在员工管理添加环境业务人员才能继续进行操作。",
              closable: true,
            });
          }
        })
    ...
    //   },
    // );
    const selectedRouteDetail = ref<RouteModel | null>(null);
