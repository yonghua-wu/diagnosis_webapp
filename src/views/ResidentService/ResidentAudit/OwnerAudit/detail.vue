<template>
  <div class="add-employess container">
    <Breadcrumb :items="['客户服务', { name: '业主审核', router: () => $router.replace({ name: 'OwnerAudit' }) }, pageModelName]" />
      ...
      <CUVue :showModalTips="showModalTips">
        <template #extra>
          <template v-if="pageModel === 'View'">
            ...
            <a-modal v-model:visible="visible" title="业主审核不通过" title-align="start" width="500px" @before-ok="handleBeforeOk">
...
export default defineComponent({
  ...
  setup() {
    ...
    const handleExamine = () => {
      ...
      apply.personUnitId = formData.value.personUnitId;
      apply.status = 1;
