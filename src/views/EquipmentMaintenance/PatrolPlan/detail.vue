<template>
  <div class="envmanagement-plan-detail container">
    <Breadcrumb :items="['设备维护', { name: '设备巡检计划', router: () => $router.replace({ name: 'EquipmentPatrolPlan' }) }, pageModelName]" />
          ...
          <a-space direction="vertical" fill>
            <a-space direction="vertical" fill>
              ...
              <a-row class="section-body" :gutter="24">
                ...
                <a-col :span="8">
                  <a-form-item required label="巡检事件" :row-props="{ gutter: 24 }">
                    <a-select
                      placeholder="请选择巡检事件"
...
import MechanicalElectricalPlan, { CreateMechanicalElectricalPlanModel, CyclicalPlanTaskModel } from "@/api/tenant/MechanicalElectricalPlan";
...
export default defineComponent({
  ...
  setup() {
    ...
    const loadData = () => {
      loading.value = true;
      stopWatch.value?.();
      const p1 = MechanicalElectricalPlan.detail(route.params.id as string).then((res) => {
        formData.value = res;
    ...
    JobConfig.all({ product: 11, serviceType: 1, serviceClass: 3 })
      .then((res) => {
    ...
    const createData = () => {
      ...
      const submitData: CreateMechanicalElectricalPlanModel = {
      ...
      return MechanicalElectricalPlan.create(submitData)
        .then(() => {
          router.push({ name: "EquipmentPatrolPlan" });
        })
        .catch((err) => {
    ...
    const updateData = () => {
      ...
      createLoading.value = true;
      console.log("submitData", submitData);
      return MechanicalElectricalPlan.update(submitData)
        .then(() => {
          router.push({ name: "EquipmentPatrolPlan" });
        })
        .catch((err) => {
    ...
    if (formData.value.serviceClass) {
      MechanicalElectricalRoute.getAll({ serviceClass: formData.value.serviceClass }).then((res) => {
      ...
      Staff.findByServiceClass(formData.value.serviceClass)
        .then((res) => {
          ...
          if (staffList.value.length === 0) {
            Modal.warning({
              title: "未配置机电人员",
              content: "需要先在员工管理添加机电业务人员才能继续进行操作。",
              closable: true,
            });
          }
        })
    ...
    //   },
    // );
    const selectedRouteDetail = ref<MechanicalElectricalRouteModel | null>(null);
