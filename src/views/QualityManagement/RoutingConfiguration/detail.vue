<template>
  <div class="routing-management-detail container">
    <Breadcrumb :items="['品质管理', { name: '路线配置', router: () => $router.replace({ name: 'QualityRoutingConfiguration' }) }, pageModelName]" />
    <a-spin :loading="loading" style="width: 100%">
      <CUVue :showModalTips="showModalTips">
        <a-form
          ...
          style="width: 500px"
        >
          <a-form-item :rules="rules.routeName" field="routeName" label="路线名称">
            <a-input :max-length="50" v-model="formData.routeName" placeholder="请输入路线名称" />
          </a-form-item>
          <a-form-item :rules="rules.serviceClass" field="serviceClass" label="业务" :disabled="pageModel === 'View' || pageModel === 'Edit'">
          ...
          <a-form-item :rules="rules.addRouteSpots" :validate-trigger="['input']" field="addRouteSpots" label="地点">
...
import QualityCheckRoute, { CreateQualityCheckRouteModel, UpdateQualityCheckRouteModel } from "@/api/tenant/QualityCheckRoute";
...
export default defineComponent({
  ...
  setup() {
    ...
    const loadData = () => {
      loading.value = true;
      stopWatch.value?.();
      QualityCheckRoute.detail(route.params.id as string)
        .then((res) => {
          formData.value = res;
    ...
    const createData = () => {
      const submitData: CreateQualityCheckRouteModel = {
      ...
      return QualityCheckRoute.create(submitData)
        .then(() => {
          router.push({ name: "QualityRoutingConfiguration" });
        })
        .finally(() => {
    ...
    const updateData = () => {
      const submitData: UpdateQualityCheckRouteModel = {
      ...
      console.log("submitData", submitData);
      return QualityCheckRoute.update(submitData)
        .then(() => {
          router.push({ name: "QualityRoutingConfiguration" });
        })
        .finally(() => {
