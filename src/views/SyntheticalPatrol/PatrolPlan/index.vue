...
import PatrolPlan, { PatrolPlanModel, PatrolPlanQueryModel } from "@/api/tenant/PatrolPlan";
...
export default defineComponent({
  ...
  setup() {
    ...
    const { formatLoadData } = useFormatData<PatrolPlanModel>((item) => {
      return {
        key: item.id,
        data: item,
        planName: item.planName,
        routeName: item.cyclicalPatrolRoutes.map((v) => v.routeName).reduce((a, b) => `${a}，${b}`),
    ...
    const formatQueryData = (_query: Record<string, any>) => {
      interface Query extends PatrolPlanQueryModel {
        [keys: string]: any;
      }
    ...
    const loadData = (current: number, pageSize: number) => {
      return PatrolPlan.page(current, pageSize, formatQueryData(fQueryData.value));
    };
    const del = (selectedKeys: string[]) => {
      return Promise.all(
        selectedKeys.map((id) => {
          return PatrolPlan.delete(id);
        }),
      ).then(() => {
    ...
    const changeStatus = (record: any) => {
      if (record.data.status === 2) {
        console.log("开");
        PatrolPlan.enable(record.data.id).then(() => {
          Message.success("成功");
          refSfTablePage.value.reLoadData();
        });
      } else {
        console.log("关");
        PatrolPlan.disable(record.data.id).then(() => {
          Message.success("成功");
          refSfTablePage.value.reLoadData();
        });
      }
